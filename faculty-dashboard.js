    ...
    const bodyRows = rows.map((interest) => {
        ...
        return `
            <div class="student-row">
                ...
                <span class="student-cell">Sem ${student.semester}</span>
...
function openReportDetailModal(reportId) {
    ...
    const teamHtml = team.length
        ? `<div class="modal-team">${team.map((m) => `<span class="team-chip">${m.name} · Sem ${m.semester}</span>`).join("")}</div>`
...
let unmanagedDeptFilter = "All";
let unmanagedDomainFilter = "All";

function renderUnmanagedFilters() {
    const departments = ["All", ...new Set(UNMANAGED_STUDENTS.map((s) => s.department))];
    ...
    document.getElementById("unmanagedFilters").innerHTML = `
        <select id="unmanagedDeptSelect">
            ${departments.map((d) => `<option value="${d}" ${d === unmanagedDeptFilter ? "selected" : ""}>${d === "All" ? "All Departments" : d}</option>`).join("")}
        </select>
    ...
    document.getElementById("unmanagedDeptSelect").addEventListener("change", (e) => {
        unmanagedDeptFilter = e.target.value;
        renderUnmanaged();
    });
    ...
    return `
        <article class="unmanaged-card">
            ...
            <div class="unmanaged-card-actions">
                ...
                <button class="btn btn-secondary" data-unmanaged-suggest="${student.id}">Suggest Project</button>
...
function renderUnmanaged() {
    const students = getUnmanagedStudents().filter((s) =>
        (unmanagedDeptFilter === "All" || s.department === unmanagedDeptFilter) &&
    ...
    modalBody.innerHTML = `
        ...
        <div class="modal-meta-row">
            <div class="modal-meta-item">
                ...
                <span class="meta-value">${student.semester}</span>
            </div>
            <div class="modal-meta-item">
...
function suggestProject(studentId) {
    const student = UNMANAGED_STUDENTS.find((s) => s.id === studentId);
    if (!student) return;
    showToast(`A project suggestion was sent to ${student.name} ✓`);
}
...
function renderTeams() {
    ...
    document.getElementById("teamsList").innerHTML = projects.map((project) => {
        const team = project.team || [];
            ...
            return `
                <div class="team-card">
                    <div class="team-card-head">
                        <p class="team-card-title">${project.title}</p>
                        <span class="badge ${statusBadgeClass(project.status)}">${project.status}</span>
                    </div>
                    <p class="team-empty">No students on this project yet.</p>
                </div>
            `;
        }
        ...
        const bySemester = {};
        team.forEach((member) => {
            const sem = member.semester || "—";
            if (!bySemester[sem]) bySemester[sem] = [];
            bySemester[sem].push(member);
        });

        const semesterGroupsHtml = Object.keys(bySemester)
            .sort((a, b) => b - a)
            .map((sem) => `
                <div class="team-semester-group">
                    <p class="team-semester-label">Semester ${sem}</p>
                    <div class="team-member-chips">
                        ${bySemester[sem].map((m) => `<span class="team-chip">${m.name}</span>`).join("")}
                    </div>
                </div>
            `).join("");
        ...
        `;
    }).join("") || `<p class="empty-state">You don't have any projects yet.</p>`;
}
...
function renderProfile() {
    ...
    document.getElementById("profileMeta").textContent = "Faculty Mentor";
...
function openDetailModal(projectId) {
    ...
    modalBody.innerHTML = `
        ...
        <div class="modal-meta-row">
            ...
            <div class="modal-meta-item">
                <span class="meta-label">Interested</span>
                <span class="meta-value">${pendingCountFor(project.id)}</span>
            </div>
        </div>
...
document.addEventListener("click", (e) => {
    ...
    const unmanagedSuggest = e.target.closest("[data-unmanaged-suggest]");
    if (unmanagedSuggest) { suggestProject(unmanagedSuggest.dataset.unmanagedSuggest); return; }
